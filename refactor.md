...
- 2단계 사용자 설정 파일과 공통 로더 추가: 완료
- 다음 작업: 3단계 사이트·SEO·내비게이션 하드코딩 제거
...
다음 작업은 **3단계: 사이트·SEO·내비게이션 하드코딩 제거**다. 2단계에서 추가한
검증된 설정 객체로 기존 소비자를 전환한다. Astro config, 화면 컴포넌트, RSS와
검증 스크립트가 같은 사이트 URL과 언어 설정을 사용하게 만든 뒤 기존 메타데이터와
114개 게시물 경로가 그대로인지 비교한다.
