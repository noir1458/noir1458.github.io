...
> 목표는 구형 Windows 노트북을 집에 덮어둔 채 **절전 상태로 대기**시키고,
> 집에서는 Mac에서 한 번 클릭, 외부에서는 **VPN → 공유기 WOL → Parsec**까지 한 번에 실행해 필요할 때 원격으로 Windows 환경을 사용할 수 있게 만드는 것이었다.
>
> 용도는 개발, 개인 공부, 파일 작업, Windows 전용 프로그램 실행 등 무엇이든 될 수 있다. 이 글은 특정 작업 용도보다 **WOL과 원격 접속을 이용해 구형 Windows 노트북을 원격 머신으로 재활용하는 과정** 자체에 초점을 둔다.
>
...
단축어 하나가 **현재 접속한 공유기가 집 공유기인지 자동으로 판별**하면 된다.
...
형태의 꽤 쓸 만한 **원격 Windows 머신**으로 바뀌었다.
